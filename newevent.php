      ...
      <div id="content">
		...
		<?php
			...
			?>
					...
					$stime = $_POST['stime']; 
					$a= mysql_query("SELECT stdate, stime, elocation from event");
					while($r = mysql_fetch_array($a, MYSQL_ASSOC)) {
					$stdatesave = $r['stdate'];
					$sttimesave = $r['stime'];
					$elocation = $r['elocation'];
					echo"$stdate, $stdatesave, $stime, $sttimesave, $eloc, $elocation";
						   if ($stdate == $stdatesave){
						   		if($stime == $sttimesave){
						   			if($eloc == $elocation){
						   			$wrong="true";
								  	 echo "<script language='javascript'>";
								   	 echo "alert('The booking date and time is not available.');";
					...
					$sql="INSERT INTO event (ename, edesc, staffname, stdate, stime, endate, entime, elocation, resc, notes)
					VALUES
					('$_POST[eventname]','$_POST[edesc]','$username', '$_POST[stdate]', '$_POST[stime]', '$_POST[endate]', '$_POST[entime]', '$_POST[eventlocation]','$_POST[resc]','$_POST[notes]')"; 
